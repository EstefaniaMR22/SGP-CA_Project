...
public class WorkPlanEditableController implements Initializable{
    ...
    private void saveWorkPlan(ActionEvent event){
        try{
            ...
            if(WORKPLAN_DAO.addWorkPlan(this.workplan)){
                WindowControllerB.getWindowController().showInfoAlert(event, "El plan de trabajo ha sido registrado exitosamente");
            }else{
                WindowControllerB.getWindowController().showErrorAlert(event, "El sistema ha presentado un error, favor de ponerse en contacto con soporte técnico");
            }
            FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("WorkPlanRequest.fxml", btnCancelChanges);
    ...
    private void updateWorkPlan(ActionEvent event){
        try{
            ...
            if(WORKPLAN_DAO.updateWorkPlan(this.workplan, this.oldWorkPlanClone)){
                WindowControllerB.getWindowController().showInfoAlert(event, "El plan de trabajo ha sido actualizado exitosamente");
            }else{
                WindowControllerB.getWindowController().showErrorAlert(event, "El sistema ha presentado un error, favor de ponerse en contacto con soporte técnico");
            }
            FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("WorkPlanRequest.fxml", btnCancelChanges);
    ...
    private void cancelChanges(ActionEvent event){
        Optional<ButtonType> action = WindowControllerB.getWindowController().showConfirmacionAlert(event, "¿Seguro que deseas cancelar los cambios?");
