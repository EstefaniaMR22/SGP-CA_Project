...
public class IntegrantRequestController implements Initializable {
    ...
    public void showIntegrantByEmail(Integrant integrantToken, String emailUV){
        ...
        if(integrant != null){
            ...
            WindowControllerB.getWindowController().showErrorAlert(new ActionEvent(), "Lo sentimos, parece que el sistema no está disponible por el momento");
    ...
    private void unsubscribeIntegrant(ActionEvent event){
        if(INTEGRANT_DAO.unsubscribeMemberByEmailUV(integrant.getEmailUV())){
            WindowControllerB.getWindowController().showInfoAlert(event, "El integrante ha sido dado de baja");
        }else{
            WindowControllerB.getWindowController().showErrorAlert(event, "Error del sistema, favor de contactas a soporte técnico");
        }
        FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("GeneralResumeRequest.fxml", btnExit);
        GeneralResumeRequestController controller = loader.getController();
    ...
    private void subscribeIntegrant(ActionEvent event){
        if(INTEGRANT_DAO.subscribeMemberByEmailUV(integrant.getEmailUV())){
            WindowControllerB.getWindowController().showInfoAlert(event, "El integrante ha sido dado de alta");
        }else{
            WindowControllerB.getWindowController().showErrorAlert(event, "Error del sistema, favor de contactas a soporte técnico");
        }
        FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("GeneralResumeRequest.fxml", btnExit);
    ...
    private void updateIntegrant(ActionEvent event){
        FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("IntegrantEditable.fxml", btnExit);
