...
public class MeetingRequestController implements Initializable{
    ...
    private void cancelMeeting(ActionEvent event){
        if(meeting.getStatusMeeting().compareTo("Pendiente") == 0){
            ...
            WindowControllerB.getWindowController().showInfoAlert(event, "Esta reunión no se puede cancelar");
        }
    }
    ...
    private void closeMeetingWindow(ActionEvent event){
        ...
        FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("MeetingHistory.fxml", this.btnCloseMeetingWindow);
    ...
    private void deleteComment(ActionEvent event){
        ...
        if(this.token.getFullName().equals(commentDeleted.getCommentator())){
            ...
            WindowControllerB.getWindowController().showInfoAlert(event, "No puede eliminar los comentarios de otros integrantes.");
        }
    }

    @FXML
    private void downloadMinute(ActionEvent event){
        WindowControllerB.getWindowController().showInfoAlert(event, "Lo sentimos. La minuta no esta disponible por el momento");
    }
    ...
    private void startMeeting(ActionEvent event) {
        if(meeting.getStatusMeeting().compareTo("Pendiente") == 0){
            ...
            WindowControllerB.getWindowController().showInfoAlert(event, "Esta reunión ya no se puede iniciar");
        }
    }
    ...
    private void updateMeeting(ActionEvent event){
        ...
        if((statusMeeting.compareTo("Pendiente") == 0) || (statusMeeting.compareTo("Cancelada") == 0)){
            MEETING_DAO.updateMeeting(meeting, meeting);
            FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("MeetingEdit.fxml", this.btnCloseMeetingWindow);
        ...
        if(statusMeeting.compareTo("Realizada") == 0){
            MEETING_DAO.updateMeeting(meeting, meeting);
            FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("MeetingRealizedEdit.fxml", this.btnCloseMeetingWindow);
    ...
    private void comproveMeetingDifferentNull(Meeting meeting){ 
        if (meeting == null){
            WindowControllerB.getWindowController().showErrorAlert(new ActionEvent(), "Lo sentimos,no se pudo encontrar la información de la reunión");
            FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("MeetingHistory.fxml", this.btnCloseMeetingWindow);
            MeetingHistoryController controller = loader.getController();
            controller.receiveToken(token);
        }
