...
public class GeneralResumeEditableController implements Initializable{
    ...
    private void signUpBodyAcademy(ActionEvent event){
        try{
            ...
            if(GENERAL_RESUME_DAO.addGeneralResume(generalResume)){
                ...
                WindowControllerB.getWindowController().showErrorAlert(event, "Error del sistema, favor de contactar a soporte técnico");
                FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("Login.fxml", btnCancelChanges);
            }
    ...
    private void addResponsibleInGeneralResumeRegistered(GeneralResume generalResume, Event event){
        ...
        if(INTEGRANT_DAO.updateMember(token, token.getEmailUV())){
            WindowControllerB.getWindowController().showInfoAlert(event, "El currículum general ha sido registrado con éxito");
            FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("Start.fxml", btnCancelRegistration);
            StartController controller = loader.getController();
            controller.receiveIntegrantToken(token);
        }else{
            FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("Login.fxml", btnCancelRegistration);
        }
    }
    ...
    private void updateGeneralResume(ActionEvent event){
        try{
            ...
            if(GENERAL_RESUME_DAO.updateGeneralResume(generalResume, this.token.getBodyAcademyKey())){
                this.token.setBodyAcademyKey(generalResume.getBodyAcademyKey());
                WindowControllerB.getWindowController().showInfoAlert(event, "El currículum ha sido actualizado exitosamente");
            }else{
                WindowControllerB.getWindowController().showErrorAlert(event, "Error del sistema, favor de contactar a soporte técnico");
            }
            FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("GeneralResumeRequest.fxml", btnCancelChanges);
    ...
    private void cancelChanges(ActionEvent event){
        ...
        if(action.get() == ButtonType.OK){
            FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("Start.fxml", btnCancelChanges);
            StartController controller = loader.getController();
    ...
    private void cancelSignUpBodyAcademy(ActionEvent event){
        Optional<ButtonType> action = WindowControllerB.getWindowController().showConfirmacionAlert(event, "¿Seguro que deseas cancelar el registro?");
        if(action.get() == ButtonType.OK){
            FXMLLoader loader = WindowControllerB.getWindowController().changeWindow("Login.fxml", btnCancelChanges);
        }
    }
